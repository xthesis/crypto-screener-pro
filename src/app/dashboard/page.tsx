...
export default function Dashboard() {
  ...
  const topLosers = [...coins].sort((a, b) => a.price_change_pe
...
export default function Dashboard() {
  ...
  const runFormula = async (formula: typeof SAVED_FORMULAS[0]) => {
    setRunningFormula(formula.id);
    try {
      const res = await fetch('/api/screen', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ conditions: formula.conditions }),
      });
      const data = await res.json();
      setFormulaResults(prev => ({ ...prev, [formula.id]: data.results }));
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const Skeleton = () => (
    <div className="space-y-3">
      {[1,2,3].map(i => <div key={i} className="h-10 rounded" style={{ background: 'linear-gradient(90deg, #26262a 25%, #333 50%, #26262a 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.5s infinite' }}></div>)}
    </div>
  );
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="card p-5">
            <div className="text-xs text-gray-500 uppercase mb-1">Total Coins</div>
            <div className="text-2xl font-bold">{loading ? '—' : coins.length}</div>
          </div>
          <div className="card p-5">
            <div className="text-xs text-gray-500 uppercase mb-1">Oversold (RSI &lt;30)</div>
            <div className="text-2xl font-bold text-green-500">{loading ? '—' : oversoldCount}</div>
          </div>
          <div className="card p-5">
            <div className="text-xs text-gray-500 uppercase mb-1">Overbought (RSI &gt;70)</div>
            <div className="text-2xl font-bold text-red-500">{loading ? '—' : overboughtCount}</div>
          </div>
          <div className="card p-5">
            <div className="text-xs text-gray-500 uppercase mb-1">Saved Formulas</div>
            <div className="text-2xl font-bold">{SAVED_FORMULAS.length}</div>
          </div>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
              ...
              <div className="space-y-3">
                {topGainers.map(c => (
                  <div key={c.id} className="flex items-center justify-between">
                    ...
                    <span className="text-green-500 font-semibold text-sm font-mono">+{c.price_change_percentage_24h.toFixed(2)}%</span>
                  </div>
                ))}
              </div>
            )}
              ...
              <div className="space-y-3">
                {topLosers.map(c => (
                  <div key={c.id} className="flex items-center justify-between">
                    ...
                    <span className="text-red-500 font-semibold text-sm font-mono">{c.price_change_percentage_24h.toFixed(2)}%</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="card p-5">
            <h3 className="text-sm font-semibold text-yellow-500 mb-3">⚡ Oversold Coins (RSI &lt;30)</h3>
            {loading ? <Skeleton /> : (
              <div className="space-y-3">
                {coins.filter(c => (c.rsi_14 ?? 50) < 30).slice(0, 3).map(c => (
                  <div key={c.id} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      {c.image && <img src={c.image} alt={c.name} width={22} height={22} className="rounded-full" />}
                      <span className="font-semibold text-sm">{c.symbol}</span>
                ...
                {coins.filter(c => (c.rsi_14 ?? 50) < 30).length === 0 && (
                  <p className="text-gray-500 text-sm">No oversold coins right now</p>
                )}
              </div>
            )}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {SAVED_FORMULAS.map(f => (
            <div key={f.id} className="card p-5">
              ...
              {formulaResults[f.id] && (
                <p className="text-xs text-green-400 mb-2 font-semibold">{formulaResults[f.id].length} coins matched</p>
              )}
                ...
                <div className="space-y-1.5 mb-3">
                  {formulaResults[f.id].slice(0, 3).map(c => (
                    <div key={c.id} className="flex items-center justify-between text-sm">
                      ...
                      <span className={`font-mono text-xs ${c.price_change_percentage_24h >= 0 ? 'text-green-500' : 'text-red-500'}`}>
              ...
              <button
                ...
                style={{ opacity: (runningFormula === f.id || loading) ? 0.5 : 1 }}
              >
